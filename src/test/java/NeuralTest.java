...
public class NeuralTest {
    ...
    //步长(学习率)
    private static final Double stepLength = 0.05;
    //训练次数
    private static final int cycleTimes = 30000;
    ...
    private static LinkedList<LinkedList<Double>> getResultMatrix(Double[][] standardResult){
        LinkedList<LinkedList<Double>> result = new LinkedList<>();
        for (Double[] doubles:standardResult){
    ...
    private static void deepLearning(InputLayer inputLayer, HiddenLayer hiddenLayer, OutputLayer outputLayer){
        ...
        LinkedList<LinkedList<Double>> results = getResultMatrix(standardResult);
...
//                System.out.println("sample:"+j+" "+samples.get(j));
//                System.out.println("forwarded:"+j+" "+resList);
                //反向传播开始
    ...
    public void testNeuralNet(){
        ...
        //输出训练后结果
        for (LinkedList<Double> sample : samples) {
            LinkedList<Double> resultByLearning = forwardPro(sample, inputLayer, hiddenLayer, outputLayer);
